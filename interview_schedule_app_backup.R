...
server <- function(input, output, session) {
  ...
  filtered_interviews <- reactive({
    ...
    filtered <- interview_data %>%
      filter(Priority %in% input$priority_filter,
             Month %in% input$month_filter)
    
    # Remove wedding dates if selected
    if (input$avoid_wedding) {
  ...
  observeEvent(input$clear_all, {
    ...
    values$selected_dates <- character()
    values$selected_programs <- character()
    showNotification("All selections cleared", type = "info", duration = 2)
  })
  ...
  output$timeline_plot <- renderPlot({
    if (nrow(values$selected_interviews) > 0) {
      ...
      ggplot(df, aes(x = Date, y = 1, label = Program)) +
      ...
      plot(1, type = "n", xlab = "", ylab = "", 
           main = "No interviews scheduled yet", 
           xlim = c(0, 1), ylim = c(0, 1))
      text(0.5, 0.5, "Select interviews to see timeline", cex = 1.5)
    }
  })
